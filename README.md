...
```bash
# Add the marketplace — directly from GitHub (recommended)
/plugin marketplace add github:AimoreRRD/my-claude-for-legal

# Install a plugin — pick the ones that match your practice
/plugin install commercial-legal@my-claude-for-legal
/plugin install privacy-legal@my-claude-for-legal
/plugin install corporate-legal@my-claude-for-legal

# Restart Claude Code, then run setup for each plugin you installed.
# This writes your practice profile to ~/.claude/plugins/config/my-claude-for-legal/<plugin>/CLAUDE.md
...
| **Practice profile** | Plain-English `CLAUDE.md` describing your playbook, escalation rules, and house style. Every skill reads from it. | `~/.claude/plugins/config/my-claude-for-legal/<plugin>/CLAUDE.md` |
...
- **Edit the practice profile.** Your profile lives at `~/.claude/plugins/config/my-claude-for-legal/<plugin>/CLAUDE.md`. Edit it directly for small fixes — a wrong escalation threshold, a new integration, a policy update. It survives plugin updates.
